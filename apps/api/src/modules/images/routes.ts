...
export const memeImageRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/images/public', async (req, reply) => {
  ...
  app.get('/images/s/:shareSlug', async (req, reply) => {
  ...
  app.post('/images/s/:shareSlug/view', {
    config: {
      rateLimit: {
  ...
  app.get('/images/:imageId', { preHandler: requireAuth }, async (req, reply) => {
  ...
  app.post('/images', { preHandler: requireAuth }, async (req, reply) => {
  ...
  app.patch('/images/:imageId', { preHandler: requireAuth }, async (req, reply) => {
    const paramsParsed = MemeImageIdParamSchema.safeParse(req.params);
    if (!paramsParsed.success) {
      return reply.code(400).send({
        message: 'Invalid image id',
  ...
  app.delete('/images/:imageId', { preHandler: requireAuth }, async (req, reply) => {
    const paramsParsed = MemeImageIdParamSchema.safeParse(req.params);
    if (!paramsParsed.success) {
      return reply.code(400).send({
        message: 'Invalid image id',
