...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon, label, isActive, onClick, disabled }) => (
  <button
    ...
    className={`
      ...
      ${disabled 
          ? 'bg-slate-50/40 text-slate-500 cursor-not-allowed opacity-60 ring-1 ring-slate-200/40' 
          : isActive 
              ? 'bg-slate-100 text-slate-900 shadow-sm ring-1 ring-slate-300/60' 
              : 'bg-slate-50/50 text-slate-700 ring-1 ring-slate-200/60 hover:bg-slate-100 hover:text-slate-800'}
    `}
  >
    <Icon 
      path={icon} 
      size={window.innerWidth < 768 ? 0.85 : 1} 
      className={`transition-all duration-300 ${isActive ? 'scale-110' : 'opacity-90'}`} 
    />
    <span className={`
