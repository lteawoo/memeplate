...
## 18단계: 도구 체계 초간소화 및 통합 편집 패널 [#21]
- [ ] **MemeToolbar 개편**: [이미지, 편집, 공유] 3대 버튼으로 축소
- [ ] **통합 리스트 UI**: 텍스트/도형 구분 없는 단일 관리 목록 구축
- [ ] **레이어 기능 통합**: 목록 내 Up/Down 버튼으로 순서 조절 구현
- [ ] **패널 구조 정리**: 불필요한 레이어 전용 섹션 및 상태 제거

## 17단계: 데스크탑 상단 툴바 레이아웃 전환 [#20]
- [ ] **MemeToolbar 가로형 최적화**: 데스크탑 뷰에서도 `flex-row` 유지 및 스타일 조정
- [ ] **MemePropertyPanel 가로형 배치**: 상단 고정 및 내부 요소 수평 정렬
- [ ] **MemeEditor 레이아웃 리팩토링**: `EditorLayout` 구조 변경 및 캔버스 영역 재배치
