...
- **이유**:
  1. 기존 `fit`은 뷰포트에 정확히 맞추는 구조여서 작업 시 시각적 여유가 부족하고, 조절점/텍스트 편집 등 정밀 작업 체감이 떨어짐.
  2. 줌/해상도 정책(800 상한)은 유지한 채 기본 표시 여백만 확보하는 방식이 회귀 리스크가 가장 낮음.
- **구현 요약**:
  - `apps/web/src/components/editor/MemeCanvas.tsx`
    - `FIT_HEADROOM_RATIO = 0.94` 상수 추가
    - 모바일/데스크톱 `fitScale` 계산 결과에 `* FIT_HEADROOM_RATIO` 적용
- **검증**:
  - `pnpm --filter memeplate-web lint`
