...
## [2026-02-07] 레이어 관리 UI 독립화 및 UX 버그 수정
- **결정**: 레이어 관리 기능을 도구별 속성 패널에서 분리하여 상시 접근 가능한 섹션으로 독립시키고, 텍스트 불투명도 조절 버그를 수정함.
- **이유**:
  1. **접근성 향상**: 도구 전환과 무관하게 레이어 순서나 삭제를 언제든 조작할 수 있도록 함.
  2. **일관성 유지**: 레이어 버튼 섹션의 디자인을 일반 도구와 통일하여 시각적 이질감 제거.
  3. **데이터 정합성**: Fabric.js 객체와 React State 간의 동기화 누락으로 발생한 UI 업데이트 버그 해결.
- **변경 사항**:
  - `MemeToolbar`: 레이어 버튼을 시스템 그룹(하단)에서 기본 그룹 하단으로 이동 및 스타일 수정.
  - `MemePropertyPanel`: 레이어 목록을 속성 영역 아래에 상시 노출(토글형)되도록 리팩토링.
  - `MemeEditor`: `opacity` 상태 추가 및 텍스트/도형 선택 시 동기화 로직 보강.
