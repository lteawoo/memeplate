...
## [2026-02-13] 리사이즈 UX 표준화: 코너 자유변형 + Shift 비율고정 + Alt 중심기준
- **결정**: 코너 핸들의 기본 동작을 비율 고정에서 자유변형으로 전환하고, 보조 키 동작을 `Shift=비율 고정`, `Alt/Option=중심 기준 리사이즈`로 정의함.
- **이유**:
  1. **사용자 기대 정합성**: 일반 그래픽 편집기 UX와 동일한 기본 규칙(코너 드래그 시 가로/세로 동시 자유 조절)을 제공.
  2. **정밀 제어성**: 비율 고정과 중심 기준 변형을 필요 시에만 modifier로 호출해 기본 조작 흐름을 단순화.
  3. **회귀 최소화**: 기존 회전/고정점 계산 구조를 유지하면서 리사이즈 계산 분기만 확장해 안정적으로 반영.
- **구현 요약**:
  - `Canvas.ts` 리사이즈 계산에서 코너 강제 uniform 로직 제거.
  - `MouseEvent.shiftKey` 입력 시 리사이즈 비율 고정 계산 적용.
  - `MouseEvent.altKey` 입력 시 객체 중심(`left/top`) 고정 상태로 대칭 리사이즈 적용.
