...
## [2026-02-07] 모바일 슬림 프로퍼티 바(Slim Bar) 도입
- **결정**: 객체 선택 시 패널 높이를 45vh에서 슬림한 가로 스크롤 바로 변경하고, 속성들을 가로로 나열함.
- **이유**: 모바일의 좁은 화면에서 패널이 캔버스를 가리는 문제를 해결하여, 실시간으로 편집 결과를 확인하며 작업할 수 있는 환경(What You See Is What You Get)을 제공함.
- **방식**: 텍스트/도형 속성은 가로 스크롤 바(약 120px)로 제공하고, 배경 업로드 등 설정이 많은 도구는 기존 컴팩트 패널 방식을 유지하여 정보의 복잡도에 따라 유동적으로 대응.
