...
3. 수정 완료 후 반드시 테스트
4. **연동된 tool playwright mcp 또는 chrome extention**으로 브라우저 직접 테스트한다.
5. **시각적 검증 필수 (Strict Visual Verification)**:
   - UI/Frontend 작업은 반드시 `screenshot` 기능을 사용하여 실제 렌더링 된 이미지를 눈으로 확인해야 한다.
   - 텍스트 기반의 DOM 트리(Snapshot)나 로그만 보고 "시각적으로 확인했다"고 거짓 보고하지 않는다.
   - 스크린샷 도구 오류 시, 이를 숨기지 말고 사용자에게 솔직하게 보고한 뒤 대안을 찾는다.
6. 스크린샷과 로그는 .playwright-mcp 안에 기록
7. 테스트가 완료되면 실행한 브라우저는 완전 종료.
